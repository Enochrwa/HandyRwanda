...
class Message(Base):
    ...
    booking_id: Column[uuid.UUID] = Column(UUID(as_uuid=True), ForeignKey("bookings.id"), nullable=False)
    sender_id: Column[uuid.UUID] = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
