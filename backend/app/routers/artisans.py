...
from app.models.artisan import ArtisanProfile, Category, PortfolioPhoto, VerificationStatus, artisan_skills
...
async def update_profile(
    ...
    result = await db.execute(select(ArtisanProfile).where(ArtisanProfile.user_id == user_id))
    ...
    if not profile:
        ...
        if location_wkt: profile.location = location_wkt
        db.add(profile)
    else:
        for key, value in payload.dict(exclude_unset=True).items():
            if key not in ["latitude", "longitude"]: setattr(profile, key, value)
        if location_wkt: profile.location = location_wkt
...
async def get_my_profile(db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(UserRole.artisan))) -> Any:
    user_id = UUID(current_user["sub"])
    result = await db.execute(select(ArtisanProfile).where(ArtisanProfile.user_id == user_id))
    profile = result.scalar_one_or_none()
...
async def submit_id_verification(payload: IDVerificationRequest, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(UserRole.artisan))) -> Any:
    user_id = UUID(current_user["sub"])
    ...
    await db.execute(update(ArtisanProfile).where(ArtisanProfile.user_id == user_id).values(verification_status=VerificationStatus.pending))
    await db.commit()
...
@router.post("/skills")
async def update_skills(category_ids: List[UUID], db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(UserRole.artisan))) -> Any:
    user_id = UUID(current_user["sub"])
    await db.execute(delete(artisan_skills).where(artisan_skills.c.artisan_id == user_id))
    for cat_id in category_ids:
        await db.execute(artisan_skills.insert().values(artisan_id=user_id, category_id=cat_id))
    await db.commit()
    return {"message": "Skills updated"}

@router.post("/portfolio")
async def add_portfolio_photo(payload: PortfolioCreate, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(UserRole.artisan))) -> Any:
    user_id = UUID(current_user["sub"])
    url = await upload_image(payload.photo_base64, f"portfolio/{user_id}")
    photo = PortfolioPhoto(artisan_id=user_id, image_url=url, job_type=payload.job_type, description=payload.description)
    db.add(photo)
    await db.commit()
    return photo

@router.get("/search")
async def search_artisans(category_id: UUID | None = Query(None), latitude: float = Query(...), longitude: float = Query(...), radius_km: int = Query(default=10, le=50), page: int = Query(default=1, ge=1), db: AsyncSession = Depends(get_db)) -> Any:
    offset = (page - 1) * 20
    query = text("""
    ...
    """)
    result = await db.execute(query, {"lng": longitude, "lat": latitude, "radius_km": radius_km, "category_id": category_id, "offset": offset})
