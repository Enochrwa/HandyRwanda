...
async def register(
    ...
    try:
        user = User(
            ...
            cell=getattr(payload, "cell", None),
            village=getattr(payload, "village", None),
            street_road=getattr(payload, "street_road", None),
            house_number=getattr(payload, "house_number", None),
...
async def verify_otp(
    ...
    return AuthResponse(
        ...
        user=UserBase(
            ...
            house_number=str(user.house_number) if getattr(user, "house_number", None) else None,
            landmark=str(user.landmark) if getattr(user, "landmark", None) else None,
