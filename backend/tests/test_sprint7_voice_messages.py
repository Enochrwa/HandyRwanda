...
def test_message_create_text_only():
    from app.routers.messages import MessageCreate
    m = MessageCreate(content="Hello!")
    assert m.content == "Hello!"
    assert m.voice_note_url is None


def test_message_create_voice_only():
    from app.routers.messages import MessageCreate
    m = MessageCreate(
        content="",
...
def test_message_create_empty_fails():
    from app.routers.messages import MessageCreate
    with pytest.raises(ValidationError):
        MessageCreate(content="", voice_note_url=None)


def test_message_create_whitespace_only_fails():
    from app.routers.messages import MessageCreate
    with pytest.raises(ValidationError):
        MessageCreate(content="   ", voice_note_url=None)


def test_message_create_none_content_coerced():
    """content=None should be coerced to '' and fail (no voice url)."""
    from app.routers.messages import MessageCreate
    with pytest.raises(ValidationError):
        MessageCreate(content=None, voice_note_url=None)  # type: ignore


# ── Uploads router config ─────────────────────────────────────────────────

def test_voice_note_in_allowed_folders():
    from app.routers.uploads import ALLOWED_FOLDERS
    assert "voice_note" in ALLOWED_FOLDERS
    assert ALLOWED_FOLDERS["voice_note"] == "voice-notes"


def test_audio_mime_types_present():
    from app.routers.uploads import AUDIO_MIME_TYPES
...
def test_voice_note_max_size_larger_than_default():
    from app.routers.uploads import MAX_SIZE_BYTES, DEFAULT_MAX_SIZE_BYTES
    assert MAX_SIZE_BYTES["voice_note"] > DEFAULT_MAX_SIZE_BYTES
    assert MAX_SIZE_BYTES["voice_note"] == 10 * 1024 * 1024


def test_audio_only_types_set():
    from app.routers.uploads import AUDIO_ONLY_TYPES
    assert "voice_note" in AUDIO_ONLY_TYPES


def test_image_only_types_not_voice():
    from app.routers.uploads import IMAGE_ONLY_TYPES
...
def test_presign_invalid_upload_type():
    client = _authed_client()
    # No auth header — should fail 401/403, not 500
    r = client.post("/uploads/presign", json={"upload_type": "INVALID", "content_type": "image/jpeg"})
    assert r.status_code in (400, 401, 403, 422)


def test_message_create_schema_rejects_empty_via_api():
...
def test_msg_dict_includes_voice_fields():
...
def test_msg_dict_text_message_not_voice_only():
    from app.routers.messages import _msg_dict
    from app.models.message import Message
    import uuid
    from datetime import datetime
