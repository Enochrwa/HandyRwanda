...
export default function ArtisanEarningsScreen() {
  ...
  const withdraw = useMutation({
    mutationFn: () => api.post('/escrow/withdraw', { amount: parseInt(amount), momo_number: momoNumber }),
    onSuccess: () => {
      Toast.show({ type: 'success', text1: 'Withdrawal submitted!', text2: 'Admin will process within 24 hours.' });
      setAmount(''); setMomoNumber('');
      refetch(); refetchWithdrawals();
    },
    onError: (e: any) => Toast.show({ type: 'error', text1: e.response?.data?.detail || 'Withdrawal failed' }),
  });

  const statusColor = (s: string) => ({ held: '#F59E0B', released: '#10B981', refunded: '#EF4444', disputed: '#F97316' }[s] ?? '#6B7280');
  const withdrawStatusColor = (s: string) => ({ pending: '#F59E0B', processing: '#3B82F6', paid: '#10B981', rejected: '#EF4444' }[s] ?? '#6B7280');
      ...
      >
        ...
        {(summary?.available_for_withdrawal ?? 0) >= 1000 && (
          <View style={styles.section}>
            ...
            >
              <Text style={styles.btnText}>{withdraw.isPending ? 'Submitting…' : 'Request Payout'}</Text>
            </TouchableOpacity>
          </View>
        )}
            ...
            : (transactions as any[]).slice(0, 20).map((t: any) => (
              <View key={t.id} style={styles.listItem}>
                ...
                <View style={[styles.badge, { backgroundColor: statusColor(t.status) + '20' }]}>
                  <Text style={[styles.badgeText, { color: statusColor(t.status) }]}>{t.status}</Text>
                </View>
              </View>
            ))
          }
          ...
          {(withdrawals as any[]).length === 0
            ...
            : (withdrawals as any[]).map((w: any) => (
              <View key={w.id} style={styles.listItem}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.itemAmount}>{w.amount.toLocaleString()} RWF → {w.momo_number}</Text>
                  {w.admin_note && <Text style={styles.itemNote}>{w.admin_note}</Text>}
                  <Text style={styles.itemMeta}>
                    {w.created_at ? formatDistanceToNow(new Date(w.created_at), { addSuffix: true }) : ''}
                  </Text>
                </View>
                <View style={[styles.badge, { backgroundColor: withdrawStatusColor(w.status) + '20' }]}>
                  <Text style={[styles.badgeText, { color: withdrawStatusColor(w.status) }]}>{w.status}</Text>
                </View>
              </View>
            ))
          }
...
const styles = StyleSheet.create({
  ...
  card: { width: '48%', backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', padding: 14 },
  cardLabel: { fontSize: 11, color: '#6B7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  cardValue: { fontSize: 22, fontWeight: '800', marginTop: 4 },
  cardCurrency: { fontSize: 11, color: '#9CA3AF', marginTop: 2 },
  section: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', padding: 16, gap: 10 },
  ...
  btn: { backgroundColor: '#1B5E3B', borderRadius: 10, paddingVertical: 13, alignItems: 'center', marginTop: 4 },
  btnDisabled: { opacity: 0.5 },
  btnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  listItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderTopWidth: 1, borderTopColor: '#F3F4F6' },
