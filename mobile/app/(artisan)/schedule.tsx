...
export default function ArtisanScheduleScreen() {
  ...
  const { data: schedule } = useQuery({
    ...
    onSuccess: (data: any) => {
      if (data.schedule?.length) {
        setSlots(data.schedule.map((s: any) => ({
  ...
  const saveSchedule = useMutation({
    mutationFn: () => api.post('/schedule/slots', slots.filter((s) => activeDays.has(s.day_of_week))),
    onSuccess: () => { Toast.show({ type: 'success', text1: 'Schedule saved!' }); qc.invalidateQueries({ queryKey: ['my-schedule'] }); },
    onError: () => Toast.show({ type: 'error', text1: 'Failed to save schedule' }),
  });

  const addBlockedDate = useMutation({
    mutationFn: () => api.post('/schedule/blocked', { blocked_date: newBlockedDate, reason: newBlockedReason || undefined }),
    onSuccess: () => {
  ...
  const updateSlot = (day: number, field: 'start_time' | 'end_time', value: string) => {
    setSlots((prev) => {
      ...
      return [...prev, { day_of_week: day, start_time: '08:00', end_time: '18:00', [field]: value }];
    });
  };

  const getSlot = (day: number) =>
    slots.find((s) => s.day_of_week === day) ?? { day_of_week: day, start_time: '08:00', end_time: '18:00' };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
            ...
            return (
              <View key={day} style={[styles.dayRow, i < 6 && styles.dayRowBorder]}>
                <View style={styles.dayLeft}>
                  <Switch value={active} onValueChange={() => toggleDay(i)} trackColor={{ false: '#D1D5DB', true: '#1B5E3B' }} thumbColor="#fff" />
                  <Text style={[styles.dayName, !active && styles.dayNameOff]}>{day}</Text>
                </View>
                {active && (
                  <View style={styles.timeRow}>
                    <TextInput style={styles.timeInput} value={slot.start_time} onChangeText={(v) => updateSlot(i, 'start_time', v)} placeholder="08:00" />
                    <Text style={styles.timeSep}>–</Text>
                    <TextInput style={styles.timeInput} value={slot.end_time} onChangeText={(v) => updateSlot(i, 'end_time', v)} placeholder="18:00" />
                  </View>
                )}
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={() => saveSchedule.mutate()} disabled={saveSchedule.isPending}>
          <Text style={styles.saveBtnText}>{saveSchedule.isPending ? 'Saving…' : 'Save Schedule'}</Text>
        </TouchableOpacity>

        <Text style={styles.sectionTitle}>Block Dates</Text>
        <Text style={styles.sectionSub}>Add dates you won't be available (holidays, personal time).</Text>

        <View style={styles.card}>
          <TextInput style={styles.inputFull} placeholder="Date (YYYY-MM-DD)" value={newBlockedDate} onChangeText={setNewBlockedDate} />
          <TextInput style={[styles.inputFull, { marginTop: 8 }]} placeholder="Reason (optional)" value={newBlockedReason} onChangeText={setNewBlockedReason} />
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', overflow: 'hidden', paddingHorizontal: 16, paddingVertical: 8 },
  ...
  timeInput: { borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4, fontSize: 13, width: 60, textAlign: 'center' },
  timeSep: { color: '#6B7280', fontSize: 14 },
  offText: { fontSize: 13, color: '#9CA3AF', fontStyle: 'italic' },
  saveBtn: { backgroundColor: '#1B5E3B', borderRadius: 10, paddingVertical: 13, alignItems: 'center' },
  saveBtnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  inputFull: { borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, backgroundColor: '#F9FAFB' },
  blockedItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', borderRadius: 10, borderWidth: 1, borderColor: '#E5E7EB', padding: 12 },
