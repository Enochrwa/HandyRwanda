...
import { Star, MapPin, Award, MessageCircle, ChevronLeft } from 'lucide-react-native';
        ...
        <View className="px-6 -mt-10">
          <View className="bg-card rounded-3xl p-6 shadow-sm border border-border">
            ...
            color="#E8A020" // @ts-ignore
                fill="#E8A020" />
                <Text className="ml-1 text-sm font-bold text-accent">{profile?.profile?.average_rating || '5.0'}</Text>
              </View>
            </View>

            <View className="flex-row gap-4 mt-6">
              <View className="flex-1 items-center bg-muted/50 p-3 rounded-2xl">
                <Text className="text-primary font-bold text-lg">{profile?.profile?.years_experience || 0}</Text>
                <Text className="text-[10px] text-muted-foreground">YEARS EXP</Text>
              </View>
              <View className="flex-1 items-center bg-muted/50 p-3 rounded-2xl">
                <Text className="text-primary font-bold text-lg">{profile?.profile?.total_reviews || 0}</Text>
                <Text className="text-[10px] text-muted-foreground">REVIEWS</Text>
              </View>
              <View className="flex-1 items-center bg-muted/50 p-3 rounded-2xl">
                <Text className="text-primary font-bold text-lg">{profile?.profile?.community_score || 0}</Text>
            ...
            <View className="mt-6 border-t border-border pt-6">
              ...
              <Text className="text-muted-foreground leading-6">
                {profile?.profile?.bio || "Expert service across Kigali with focus on quality and reliability. Always ready to help with your home maintenance needs."}
              </Text>
            </View>

            <View className="mt-6 flex-row items-center">
              <MapPin size={18} // @ts-ignore
            color="#6B6B6B" />
              <Text className="ml-2 text-muted-foreground">{profile?.profile?.location_label || "Gasabo, Kigali"}</Text>
            </View>
          </View>
          ...
          <View className="mt-8">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-3">
              {profile?.portfolio?.length > 0 ? profile.portfolio.map((img: any) => (
                <Image
                  key={img.id}
