  ...
  const onSendOTP = async (data: LoginData) => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: err.response?.data?.detail ?? 'Failed to send OTP',
      });
    } finally {
      setLoading(false);
    }
  ...
  const onVerifyOTP = async (data: OTPData) => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: err.response?.data?.detail ?? 'Invalid OTP',
      });
    } finally {
      setLoading(false);
    }
  ...
  const onRegister = async (data: RegisterData) => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: err.response?.data?.detail ?? 'Registration failed',
      });
    } finally {
      setLoading(false);
    }
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="p-6">
        <View className="mt-10 mb-8">
          ...
          <Text className="text-muted-foreground text-center mt-2">
            Connecting Kigali to the best artisans
          </Text>
        </View>
        ...
        <View className="flex-row bg-muted rounded-xl p-1 mb-6">
          ...
          >
            <Text
              className={`text-center font-semibold ${activeTab === 'login' ? 'text-foreground' : 'text-muted-foreground'}`}
            >
              Login
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text
              className={`text-center font-semibold ${activeTab === 'register' ? 'text-foreground' : 'text-muted-foreground'}`}
            >
              Register
            </Text>
          </TouchableOpacity>
              ...
              <View>
                ...
                >
                  {loading ? (
                    ...
                    <Text className="text-white font-bold">Verify & Login</Text>
                  )}
                </TouchableOpacity>
              ...
              render={({ field: { onChange, value } }) => (
                <View className="mb-6">
                  ...
                  <View className="flex-row gap-2">
                    ...
                    >
                      <Text
                        className={`text-center font-bold ${value === 'client' ? 'text-primary' : 'text-muted-foreground'}`}
                      >
                        Client
                      </Text>
                    </TouchableOpacity>
                    ...
                    >
                      <Text
                        className={`text-center font-bold ${value === 'artisan' ? 'text-primary' : 'text-muted-foreground'}`}
                      >
                        Artisan
                      </Text>
                    </TouchableOpacity>
