...
/**
 * Sprint 7 enhanced — Chat thread with Voice Messages.
 *
 * Additions vs Sprint 1:
 *  - Microphone button (hold to record, release to stop) via useVoiceRecorder
 *  - VoicePreviewCard shown after recording stops (play, delete, send)
 *  - VoiceMessageBubble for messages with voice_note_url
 *  - Optimistic voice message placeholder in message list
 *  - Existing text messaging, LiveStatusCard, Socket.IO all preserved
 */
...
import { VoicePreviewCard } from '../../src/components/VoicePreviewCard';
...
function MessageItem({ item, userId }: MessageItemProps) {
  ...
  // Optimistic voice note placeholder (uploading)
  if (item._isVoicePlaceholder) {
    return (
  ...
  return (
    <View className={`mb-3 flex-row ${isMe ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        {item.translated_content && item.translated_content !== item.content && (
          <Text
...
export default function ChatThread() {
  ...
  useEffect(() => {
    if (voiceError) {
      Toast.show({ type: 'error', text1: 'Voice recording error', text2: voiceError });
    }
  }, [voiceError]);
  ...
  const sendMutation = useMutation({
    mutationFn: (text: string) =>
      api.post(`/messages/${bookingId}`, { content: text }),
  ...
  // ── Voice message sent callback ───────────────────────────────────────

  const handleVoiceSent = useCallback(() => {
    discardRecording();
    qc.invalidateQueries({ queryKey: ['messages', bookingId] });
    qc.invalidateQueries({ queryKey: ['conversations'] });
    Toast.show({ type: 'success', text1: '🎙️ Voice message sent!' });
  }, [discardRecording, qc, bookingId]);
  ...
  const confirmPayment = useMutation({
    ...
    onSuccess: () => {
      Toast.show({
        ...
        text2: 'Artisan notified. 15-min accept window started.',
      });
      qc.invalidateQueries({ queryKey: ['booking-detail', bookingId] });
    },
  ...
  const raiseDispute = useMutation({
    mutationFn: () =>
      api.post(`/bookings/${bookingId}/dispute`, {
        reason: 'Issue raised via app',
      }),
    onSuccess: () => {
      Toast.show({
        type: 'info',
        text1: '⚠️ Dispute raised',
        text2: 'Admin will review shortly.',
      });
      qc.invalidateQueries({ queryKey: ['booking-detail', bookingId] });
    },
  });
  ...
  const handleDispute = () => {
    Alert.alert(
      'Raise Dispute',
      'Are you sure you want to raise a dispute for this booking?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Raise Dispute',
          style: 'destructive',
          onPress: () => raiseDispute.mutate(),
        },
      ],
    );
  };
  ...
  const showPreview = voiceState === 'stopped' && recordingUri;

  // Decide whether to show mic or send button
  const showMicButton = !content.trim();
  ...
  return (
    ...
    >
            ...
            {currentStatus && (
              <View className="flex-row items-center gap-1 mt-0.5">
                <View
                  ...
                  style={{
                    backgroundColor: STATUS_COLORS[currentStatus] ?? '#6B6B6B',
                  }}
                />
      ...
      {showLiveCard && bookingId && (
        <View className="px-4 pt-3">
          <LiveStatusCard
            ...
            artisanName={liveArtisanName || booking?.artisan?.name || ''}
            isArtisan={!!isArtisan}
      ...
      {/* ── Voice Preview Card (shown when recording stopped) ───────── */}
      {showPreview && (
        <VoicePreviewCard
          uri={recordingUri!}
      ...
      {!chatDisabled && !showPreview && (
        <View className="px-4 py-3 bg-card border-t border-border">
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8 }}>
            ...
            {/* Send button — only when text present and not recording */}
            {!isRecording && (
              <TouchableOpacity
                onPress={() => {
