...
import { formatRWF } from "@/lib/data";
...
export const Route = createFileRoute("/pro")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track your earnings, schedule, and nearby jobs. Built for working artisans." },
    ],
  }),
  component: Pro,
});
  ...
  return (
    <div className="min-h-dvh">
      ...
      <main className="mx-auto max-w-4xl px-4 pb-16 pt-6 sm:px-6">
        <p className="text-sm font-semibold uppercase tracking-wider text-primary">Muraho, Jean-Pierre 👋</p>
        ...
        >
          <div>
            ...
            <div className="mt-1 text-lg font-bold">{online ? "Available now" : "Tap to go online"}</div>
          </div>
          <div className={[
        ...
        <section className="mt-5 rounded-2xl border border-border bg-card p-6 shadow-card">
          <div className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">This month</div>
          <div className="mt-1 text-4xl font-extrabold text-success">{formatRWF(47500)} <span className="text-lg font-semibold text-muted-foreground">RWF</span></div>
          ...
          <svg viewBox="0 0 200 50" className="mt-4 h-14 w-full" preserveAspectRatio="none">
            ...
            <path d="M0,38 L25,30 L50,32 L75,18 L100,22 L125,12 L150,16 L175,8 L200,14 L200,50 L0,50 Z" fill="url(#g)" />
            <path d="M0,38 L25,30 L50,32 L75,18 L100,22 L125,12 L150,16 L175,8 L200,14" fill="none" stroke="oklch(0.42 0.1 152)" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
          </svg>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {[
              ...
              <span key={label} className="inline-flex items-center gap-1.5 rounded-full bg-accent/15 px-3 py-1.5 text-xs font-bold">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <article key={i} className="rounded-2xl border border-border bg-card p-4 shadow-card">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Client budget: <span className="font-semibold text-foreground">{formatRWF(j.budget)} RWF</span></p>
        ...
        <section className="mt-10 rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 to-accent/10 p-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold">Artisans on HandyRwanda earn on average {formatRWF(120000)} RWF / month</h2>
          </div>
          <p className="mt-2 text-sm text-muted-foreground">Complete your profile to start receiving jobs.</p>
          <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-muted">
            <div className="h-full w-3/4 bg-primary" />
          </div>
          <p className="mt-2 text-xs font-semibold text-muted-foreground">Profile 75% complete · add a portfolio photo to finish</p>
        </section>
      </main>
    </div>
  );
...
function ScheduleRow({ time, title, status }: { time: string; title: string; status: "confirmed" | "pending" | "free" }) {
  return (
  ...
  return (
    <li className="flex items-center gap-3">
      <span className={[
        ...
        done ? "border-success bg-success text-card" : active ? "border-success bg-success/20 animate-pulse-dot" : "border-border bg-card",
      ].join(" ")}>
        {done && <span className="text-[10px] font-extrabold">✓</span>}
      </span>
      <span className={[
        "text-sm",
        done ? "font-semibold text-foreground line-through decoration-success/40" : active ? "font-bold text-foreground" : "text-muted-foreground",
      ].join(" ")}>
        {label}
      </span>
    </li>
