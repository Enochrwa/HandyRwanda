...
export const Route = createFileRoute("/search")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Search verified plumbers, electricians, cleaners and more across Kigali." },
...
function SearchPage() {
  ...
  const results = artisans
    .filter((a) => (q ? (a.name + a.category + a.district).toLowerCase().includes(q.toLowerCase()) : true))
    .filter((a) => (active === "Available Now" ? a.availableNow : active === "Verified Only" ? a.verified : active === "Pro" ? a.pro : true))
    ...
    <div className="min-h-dvh">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-6 sm:px-6">
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 shadow-card">
          ...
          />
          <button aria-label="Filters" className="grid h-10 w-10 place-items-center rounded-xl bg-muted text-foreground">
        ...
        {view === "map" ? (
          <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_360px]">
            <div className="relative h-[520px] overflow-hidden rounded-2xl border border-border bg-[oklch(0.92_0.02_140)] shadow-card">
              ...
              {results.map((a, i) => (
                <div key={a.id} className="absolute" style={{ left: `${20 + i * 18}%`, top: `${30 + (i % 2) * 25}%` }}>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {results.length === 0 ? (
              <div className="col-span-full rounded-2xl border border-dashed border-border bg-card p-10 text-center">
                ...
                <p className="text-sm text-muted-foreground">No one matches your search right now.</p>
                <div className="mt-4 flex justify-center gap-2">
                  <button className="rounded-xl bg-primary px-4 py-2.5 font-semibold text-primary-foreground">Post a job</button>
                  <button className="rounded-xl border border-border bg-card px-4 py-2.5 font-semibold">Expand radius</button>
                </div>
              </div>
            ) : (
              results.map((a) => <ArtisanCard key={a.id} a={a} />)
