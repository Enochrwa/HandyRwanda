          ...
          {done ? (
            <div className="text-center py-4">
              ...
              <p className="mt-2 text-muted-foreground text-sm">
                Your request has been sent to {a.name.split(" ")[0]}. They'll bid on your job
                shortly.
              </p>
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-1.5 max-h-40 overflow-y-auto">
                    {categories.map((cat) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-[10px] font-semibold mt-0.5 ${selectedCategoryId === cat.id ? "text-primary" : "text-foreground"}`}
                        >
                          {cat.name_en}
                        </span>
                      </button>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {URGENCY_OPTIONS.map((u) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-bold mt-0.5 ${urgency === u.value ? "text-primary" : "text-foreground"}`}
                      >
                        {u.label}
                      </span>
                    </button>
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-1.5 block">
                  ...
                  Preferred date/time <span className="text-xs font-normal">(optional)</span>
                </label>
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-1.5 block">
                  ...
                  Budget (RWF) <span className="text-xs font-normal">— optional</span>
                </label>
                <input
                  type="number"
                  value={budget}
            ...
            <div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <SummaryRow
                  label="Urgency"
                  value={URGENCY_OPTIONS.find((u) => u.value === urgency)?.label ?? urgency}
                />
                {scheduledDate && (
                  <SummaryRow label="Scheduled" value={`${scheduledDate} at ${scheduledTime}`} />
                )}
                <SummaryRow
                  label="Budget"
                  value={
                    budget
                      ...
                      : `Open bids (starting ~${formatRWF(a.startingPrice ?? 5000)} RWF)`
                  }
                />
                {selectedCategoryId && (
                  <SummaryRow
                    label="Category"
                    value={categories.find((c) => c.id === selectedCategoryId)?.name_en ?? "—"}
                  />
                )}
              </div>
