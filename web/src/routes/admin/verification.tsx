...
import {
  ShieldCheck, XCircle, ExternalLink, User, BarChart3, Users,
  AlertTriangle, Package, Settings, LogOut, Bell, Search,
  CheckCircle, Ban, Eye, Loader2, TrendingUp,
} from "lucide-react";
    ...
    <div className="min-h-dvh bg-muted/30">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        ...
        <div className="flex overflow-x-auto gap-1 rounded-2xl bg-card border border-border p-1.5 mb-6 w-fit">
          ...
          ] as [Tab, typeof ShieldCheck, string][]).map(([id, Icon, label]) => (
...
function VerificationTab({ qc }: { qc: ReturnType<typeof useQueryClient> }) {
  ...
  const approve = useMutation({
    ...
    onSuccess: () => { toast.success("Artisan approved!"); qc.invalidateQueries({ queryKey: ["admin-pending"] }); },
  });
  const reject = useMutation({
    mutationFn: (id: string) => api.post(`/admin/artisans/${id}/reject`, { reason: "Documents unclear" }),
    onSuccess: () => { toast.error("Artisan rejected."); qc.invalidateQueries({ queryKey: ["admin-pending"] }); },
  });

  return (
    <div>
      <h2 className="text-xl font-bold mb-1">Verification Queue</h2>
      <p className="text-sm text-muted-foreground mb-4">Review artisan ID documents before approving.</p>
      {isLoading ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y">
              {pending.map((item: {
                user: { id: string; full_name: string; email: string; phone_number: string; avatar_url?: string; created_at?: string };
                ...
                <tr key={item.user.id} className="hover:bg-muted/20">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground hidden sm:block">{item.user.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell text-muted-foreground">{item.user.phone_number}</td>
                  <td className="px-5 py-4 hidden md:table-cell text-muted-foreground">
                    {item.profile.submitted_at ? new Date(item.profile.submitted_at).toLocaleDateString() : "—"}
                  </td>
                  <td className="px-5 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      {item.profile.id_document_url && (
                        <a href={item.profile.id_document_url} target="_blank" rel="noopener noreferrer"
                      ...
                      {item.profile.selfie_url && (
                        <a href={item.profile.selfie_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => approve.mutate(item.user.id)} disabled={approve.isPending}
                      ...
                      <button onClick={() => reject.mutate(item.user.id)} disabled={reject.isPending}
...
function AnalyticsTab() {
  ...
  const stats = [
    { label: "Total Users", value: data.users.total, sub: `${data.users.artisans} artisans, ${data.users.clients} clients`, color: "bg-blue-100 text-blue-700" },
    { label: "Total Bookings", value: data.bookings.total, sub: `${data.bookings.completed} completed`, color: "bg-green-100 text-green-700" },
    { label: "Dispute Rate", value: `${data.bookings.total > 0 ? ((data.bookings.disputed / data.bookings.total) * 100).toFixed(1) : 0}%`, sub: `${data.bookings.disputed} total disputes`, color: "bg-amber-100 text-amber-700" },
    { label: "Total Revenue", value: formatRWF(data.total_revenue_rwf), sub: "RWF from completed jobs", color: "bg-purple-100 text-purple-700" },
    { label: "Pending Verifications", value: data.pending_verifications, sub: "Awaiting admin review", color: "bg-orange-100 text-orange-700" },
  ];
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {stats.map((s) => (
          <div key={s.label} className="rounded-2xl border border-border bg-card p-4">
            <div className={`inline-block rounded-xl px-2 py-1 text-xs font-bold mb-2 ${s.color}`}>{s.label}</div>
      ...
      {data.top_artisans?.length > 0 && (
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="space-y-3">
            {data.top_artisans.map((a: { name: string; rating: number; jobs: number }, i: number) => (
...
function UsersTab({ search, setSearch, qc }: { search: string; setSearch: (s: string) => void; qc: ReturnType<typeof useQueryClient> }) {
    ...
    <div>
      ...
      {isLoading ? <div className="flex items-center justify-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div> : (
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {users.map((u: {
                ...
                role: string; account_status: string; created_at?: string;
              }) => (
                <tr key={u.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${u.role === "artisan" ? "bg-blue-100 text-blue-700" : u.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-green-100 text-green-700"}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${u.account_status === "active" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
                      {u.account_status}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      {u.account_status !== "suspended" ? (
                        <button onClick={() => suspend.mutate(u.id)} className="rounded-lg bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-700 hover:bg-amber-200 transition">Suspend</button>
                      ) : (
                        <button onClick={() => activate.mutate(u.id)} className="rounded-lg bg-success/10 px-2.5 py-1 text-xs font-bold text-success hover:bg-success/20 transition">Activate</button>
                      )}
                    </div>
                  </td>
                </tr>
          ...
          {users.length === 0 && <p className="p-8 text-center text-muted-foreground text-sm">No users found.</p>}
        </div>
      )}
    </div>
  );
...
function DisputesTab({ qc }: { qc: ReturnType<typeof useQueryClient> }) {
  ...
  const resolve = useMutation({
    ...
    onSuccess: () => { toast.success("Dispute resolved."); qc.invalidateQueries({ queryKey: ["admin-disputes"] }); },
  });
    ...
    <div>
      ...
      {isLoading ? <div className="flex items-center justify-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
        : disputes.length === 0 ? (
          ...
          <div className="space-y-3">
            {disputes.map((d: { booking_id: string; client_name: string; agreed_price: number; created_at?: string }) => (
              <div key={d.booking_id} className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-semibold">Booking: <span className="font-mono text-xs">{d.booking_id.slice(0, 8)}…</span></p>
                    <p className="text-sm text-muted-foreground mt-1">Client: {d.client_name} — {formatRWF(d.agreed_price)} RWF</p>
                    <p className="text-xs text-muted-foreground">{d.created_at ? new Date(d.created_at).toLocaleDateString() : ""}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => resolve.mutate({ id: d.booking_id, winner: "artisan" })}
                      className="rounded-xl bg-blue-100 px-3 py-1.5 text-xs font-bold text-blue-700 hover:bg-blue-200 transition">
                      Favour Artisan
                    </button>
                    <button onClick={() => resolve.mutate({ id: d.booking_id, winner: "client" })}
...
function CategoriesTab({ qc }: { qc: ReturnType<typeof useQueryClient> }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      toast.success("Category created!"); setForm({ name_rw: "", name_en: "", name_fr: "", icon_emoji: "" });
      ...
      <div>
        ...
        {isLoading ? <Loader2 className="h-6 w-6 animate-spin text-primary" /> : (
          <div className="space-y-2">
            {categories.map((c: { id: string; icon_emoji?: string; name_en: string; name_rw: string; name_fr: string }) => (
              <div key={c.id} className="flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{c.name_rw} · {c.name_fr}</p>
                </div>
              </div>
            ))}
      ...
      <div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
          {(["icon_emoji", "name_en", "name_rw", "name_fr"] as const).map((field) => (
            <div key={field}>
              <label className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{field.replace("_", " ")}</label>
              <input value={form[field]} onChange={(e) => setForm((p) => ({ ...p, [field]: e.target.value }))}
                placeholder={field === "icon_emoji" ? "🔧" : field === "name_en" ? "Plumbing" : field === "name_rw" ? "Gutunga amazi" : "Plomberie"}
          ...
          <button onClick={() => create.mutate()} disabled={create.isPending || !form.name_en}
