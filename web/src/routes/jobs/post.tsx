...
import { MapPin, Loader2, CheckCircle, ArrowRight, Info, Clock, DollarSign, FileText, Camera } from "lucide-react";
...
const PLACEHOLDER_DESCRIPTIONS: Record<string, string> = {
  default: "Describe the problem in detail — what happened, how long it's been going on, what you've already tried, the severity, and any special access requirements (gate code, floor number, etc.)",
  Plumbing: "e.g. The kitchen sink has been leaking under the cabinet for 3 days. Water pools on the floor. The shut-off valve still works. I need the pipe connection replaced.",
  Electrical: "e.g. Two sockets in the living room stopped working after we had a power surge. The circuit breaker trips when I try to reset it. I need a qualified electrician to diagnose and fix it.",
  Cleaning: "e.g. 3-bedroom apartment, last cleaned 2 weeks ago. I need a deep clean including inside fridge, oven, and bathroom tiles. I have cleaning products available.",
  Carpentry: "e.g. The bedroom door frame is warped and the door no longer closes properly. I need the frame realigned and the door rehung. Photos attached.",
  Painting: "e.g. Living room and two bedrooms need repainting. Walls are 2.7m high, currently magnolia. I'd like them in white. Surfaces have minor cracks to fill first.",
};
...
function PostJob() {
  ...
  const selectedCatName = categories.find((c: { id: string; name_en: string }) => c.id === formData.category_id)?.name_en || "default";
  const descriptionPlaceholder = PLACEHOLDER_DESCRIPTIONS[selectedCatName] || PLACEHOLDER_DESCRIPTIONS["default"];
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.category_id) { toast.error("Select a service category"); return; }
    if (!formData.title.trim() || formData.title.length < 5) {
      toast.error("Add a job title (at least 5 characters)"); return;
    }
    if (!formData.description.trim() || formData.description.length < 15) {
      toast.error("Describe the work in more detail (at least 15 characters)"); return;
    }
    if (!formData.location_label.trim()) {
      toast.error("Please specify a location"); return;
    }
      ...
      <div className="min-h-dvh bg-muted/30">
        ...
        <main className="mx-auto max-w-xl px-4 pt-20 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Verified artisans will see your job and start bidding. You'll be notified as bids arrive.
          </p>
      ...
      <main className="mx-auto max-w-2xl px-4 pt-8 sm:pt-12">
        ...
        <div className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8 space-y-8">
          <div>
            ...
            <p className="mt-1 text-muted-foreground text-sm">
              The more detail you provide, the more accurate and competitive the bids you'll receive.
            </p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
              {categories.map((cat: { id: string; name_en: string; icon_emoji?: string }) => (
                <button key={cat.id} onClick={() => set("category_id", cat.id)}
                  ...
                  <span className={`text-[11px] font-semibold ${formData.category_id === cat.id ? "text-primary" : "text-foreground"}`}>
                    {cat.name_en}
                  </span>
                </button>
              ))}
          ...
          <section>
            <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">
              ...
              <FileText className="h-3.5 w-3.5" /> Job Title <span className="text-destructive">*</span>
            </label>
          ...
          <section>
            <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">
              <span className="w-5 h-5 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-[10px] font-bold">4</span>
              Additional Notes <span className="text-muted-foreground text-[10px] font-normal">(optional)</span>
            </label>
            <textarea value={formData.additional_notes} onChange={(e) => set("additional_notes", e.target.value)}
          ...
          <section>
            <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">
              <span className="w-5 h-5 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-[10px] font-bold">5</span>
              <Clock className="h-3.5 w-3.5" /> How Urgently Do You Need This? <span className="text-destructive">*</span>
            </label>
            <div className="grid grid-cols-5 gap-1.5">
              {URGENCY_OPTIONS.map((u) => (
                <button key={u.value} onClick={() => setSelectedUrgency(u.value)}
                  title={u.desc}
                  ...
                  <span className={`text-[10px] font-bold mt-0.5 ${selectedUrgency === u.value ? "text-primary" : "text-foreground"}`}>{u.label}</span>
                </button>
              ))}
            </div>
          ...
          <section>
            <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">
              <span className="w-5 h-5 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-[10px] font-bold">6</span>
              Preferred Date & Time <span className="text-muted-foreground text-[10px] font-normal">(optional)</span>
            </label>
            <div className="grid grid-cols-2 gap-3">
              <input type="date" value={formData.scheduled_date} onChange={(e) => set("scheduled_date", e.target.value)}
              ...
              <input type="time" value={formData.scheduled_time} onChange={(e) => set("scheduled_time", e.target.value)}
                className="rounded-2xl border border-border bg-muted/30 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/40" />
            </div>
          </section>
          ...
          <section>
            <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">
              <span className="w-5 h-5 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-[10px] font-bold">7</span>
              Budget & Location
            </label>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={formData.budget} onChange={(e) => set("budget", e.target.value)}
                  placeholder="e.g. 15000" min={500}
                  className="w-full rounded-2xl border border-border bg-muted/30 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/40" />
                {formData.budget && (
                  <p className="text-[10px] text-primary mt-0.5">≈ {formatRWF(parseInt(formData.budget))} RWF</p>
                )}
              </div>
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground mb-1 block">
                  <MapPin className="inline h-3 w-3 mr-0.5" /> Location <span className="text-destructive">*</span>
                </label>
                <input value={formData.location_label} onChange={(e) => set("location_label", e.target.value)}
            ...
            {formData.budget && (
              <label className="flex items-center gap-2 mt-2 cursor-pointer">
                <input type="checkbox" checked={formData.budget_negotiable}
                  onChange={(e) => set("budget_negotiable", e.target.checked)}
                  className="rounded" />
                <span className="text-xs text-muted-foreground">I'm open to negotiate if needed</span>
              </label>
            )}
          </section>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                <p><strong>Tips for getting great bids:</strong></p>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5">
          ...
          <div className="space-y-3">
            {[
              { n: "1", t: "Artisans bid", d: "Verified artisans in your area see the job and submit competitive quotes." },
              { n: "2", t: "Compare bids", d: "Review artisan profiles, ratings, and prices before choosing." },
              { n: "3", t: "Accept & pay", d: "Accept the best bid. Pay via MoMo — funds held until job is confirmed." },
              { n: "4", t: "Rate & review", d: "After the job, leave a review to help others find great artisans." },
            ].map((s) => (
