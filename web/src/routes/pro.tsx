...
function Pro() {
  ...
  const confirmReceipt = useMutation({
    ...
    onSuccess: () => {
      toast.success("Receipt confirmed! Job is now in progress.");
                ...
                {bookable.map((b: Booking) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <span
                        className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${b.status === "in_progress" ? "bg-success/10 text-success" : b.status === "confirmed" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}
                      >
                        {b.status.replace("_", " ")}
                      </span>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      {b.status === "confirmed" && (
                        <Button
                          size="sm"
                          onClick={() => confirmReceipt.mutate(b.id)}
